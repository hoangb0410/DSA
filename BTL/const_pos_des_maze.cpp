...
* Giai thuat:       Tim kiem theo chieu sau ngau nhien (Randomized depth-first search) 	   *
*******************************************************************************************/
#include <iostream>
#include <ctime>
#include <windows.h>
...
void GenerateMaze(Cell Maze[][SIZE])
{
	...
	Maze[1][1].display = 'S';									// Dat S la o bat dau
	...
	while(visitedCells < totalCells)
	{
		...
		{
			random = (rand() % 4) + 1;		// Chon ngau nhien tuong tu 1 den 4 de pha
			 // DI LEN
			if((random == 1) && (randomY > 1))
			{
				if(Maze[randomY-2][randomX].visited == false)  // Neu chua duoc truy cap
				{
					...
					visitedCells++;					// Tang so o da duoc truy cap
				}
				else
					continue;
			}
			...
			percent = (visitedCells*100/totalCells*100)/100;		// Tien trinh (theo %)
	...
	// Dich den (Destination)
	Maze[SIZE-2][SIZE-2].display = 'E'; // Dat dich den la E
	system("cls");
	ClearScreen();
	Draw(Maze);
