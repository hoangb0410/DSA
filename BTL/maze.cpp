...
#include <fstream>
using namespace std;
#define SIZE 15 //Kich thuoc me lo
// Cau truc o
struct Cell
{
...
void GenerateMaze(Cell Maze[][SIZE]) 
{
	...
	int randomX = ((2*rand())+1)%(SIZE-1);						// Tao so le ngau nhien trong doan tu 1 den SIZE-1
	int randomY = ((2*rand())+1)%(SIZE-1);						// Tao so le ngau nhien trong doan tu 1 den SIZE-1
